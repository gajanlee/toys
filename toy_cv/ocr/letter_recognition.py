...
from tensorflow.keras.layers import Dense, Conv2D, MaxPooling2D, Reshape, Dropout, Flatten
...
def convert_image_array(base_dir, classes):
    ...
    for category in range(num_classes):
        ...
        for _, _, files in os.walk(dir):
            for file in files:
                image_array[index] = get_image_pixels(filepath)
...
def get_task_data(task, mode="train"):
    tasks = {
        "area": Dataset(f"./dataset/{mode}/area", "ABCDEFGHIJKLMNOPQRSTUVWXYZ",),
        "letter": Dataset(f"./dataset/{mode}/letter", "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ",),
        "province": Dataset(f"./dataset/{mode}/province", ("皖", "沪", "津", "渝", "冀", "晋", "蒙", "辽", "吉", "黑", "苏", "浙", "京", "闽", "赣", "鲁", "豫", "鄂", "湘", "粤", "桂", "琼", "川", "贵", "云", "藏", "陕", "甘", "青", "宁", "新"))
    }
...
class SimpleModel(object):
    ...
    def load(self):
        self.model.load_weights("{}_{}".format(self.task, len(self.classes)))
